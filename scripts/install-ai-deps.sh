...
# 注意：
#   1. whisper.cpp 使用 SSH 协议 clone，需先配置 GitHub SSH key（ssh -T git@github.com）
...
if [ ! -d "${WHISPER_SRC_DIR}" ]; then
  git clone --depth 1 git@github.com:ggerganov/whisper.cpp.git "${WHISPER_SRC_DIR}"
fi
cd "${WHISPER_SRC_DIR}"
...
if [ ! -f "${WHISPER_MODEL}" ]; then
  curl -fL --connect-timeout 15 -o "${WHISPER_MODEL}" \
