/**
 * 会话侧边栏组件
 * 两种形态共用同一份面板内容（SessionPanelContent）：
 * - SessionSidebar（默认导出）：<lg 断点的抽屉式面板（左侧滑出 + 遮罩）
 * - DockedSessionSidebar：≥lg 断点常驻左栏的静态面板（可折叠，折叠入口在头部）
 * 支持新建、搜索、重命名、删除。
 */
...
interface SessionPanelContentProps extends SessionListHandlers {
  /** 头部右侧操作区（抽屉为关闭按钮，常驻栏为折叠按钮） */
...
function SessionPanelContent({
  ...
  const commitRename = async () => {
    ...
    try {
      ...
      window.setTimeout(() => setRenameError(''), 4000);
    } finally {
      cancelRename();
    }
  };
...
interface SessionSidebarProps extends SessionListHandlers {
  isOpen: boolean;
  onClose: () => void;
}

/** <lg 断点的抽屉式会话面板（左侧滑出 + 遮罩），选中会话后自动收起 */
      ...
      >
        <SessionPanelContent
          ...
          onSelectSession={(sessionId) => {
            handlers.onSelectSession(sessionId);
            onClose();
          }}
          onNewSession={() => {
          ...
          headerAction={
            <IconBtn title="收起会话列表" onClick={onClose}>
...
interface DockedSessionSidebarProps extends SessionListHandlers {
  /** 折叠常驻栏（折叠入口在面板头部） */
  onCollapse: () => void;
}

/** ≥lg 断点常驻左栏的会话面板（静态定位，无遮罩） */
export function DockedSessionSidebar({ onCollapse, ...handlers }: DockedSessionSidebarProps) {
  return (
    <div
    ...
    >
      <SessionPanelContent
        {...handlers}
        headerAction={
          <IconBtn title="折叠会话列表" onClick={onCollapse}>
