...
export default function EntityResolver({ mentions, onResolved }: Props) {
  ...
  return (
    <div className="space-y-3 rounded-lg border bg-white p-3">
      <h4 className="font-medium text-slate-900">确认提到的人</h4>
      {mentions.map((mention) => {
        const list = candidates[mention.mention] || [];
        return (
          <div key={mention.mention} className="rounded-md bg-slate-50 p-3">
            <div className="flex items-center justify-between">
              <span className="font-medium">“{mention.mention}”</span>
              <span className="text-xs text-slate-500">置信度 {Math.round(mention.confidence * 100)}%</span>
            </div>
            {list.length === 0 ? (
              <p className="mt-2 text-sm text-slate-500">没有找到候选联系人，可先保存原始记录。</p>
            ) : (
              <div className="mt-2 flex flex-wrap gap-2">
                {list.map((person) => (
                  <button
                    key={person.id}
                    type="button"
                    className={`rounded border px-3 py-1 text-sm ${resolved[mention.mention] === person.id ? 'border-blue-600 bg-blue-600 text-white' : 'bg-white text-slate-700'}`}
                    onClick={() => setChoice(mention.mention, person.id)}
                  >
                    {person.name} {person.company ? `｜${person.company}` : ''}
                  </button>
                ))}
                <button type="button" className="rounded border bg-white px-3 py-1 text-sm text-slate-500" onClick={() => setChoice(mention.mention, null)}>
                  忽略
                </button>
              </div>
            )}
